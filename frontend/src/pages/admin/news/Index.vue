...
// LƯU Ý: Đã gỡ bỏ import vue-quill và html2pdf.js để tránh lỗi Vite (500 Internal Server Error)
// vì dự án của bạn chưa chạy lệnh npm install cho 2 thư viện này.

// CONFIGURATION
const apiUrl = import.meta.env.VITE_API_BASE_URL || 'http://127.0.0.1:8000/api';
// Lấy URL Frontend để chia sẻ link
const FRONTEND_URL = window.location.origin;
// Đường dẫn Backend để load ảnh
const BACKEND_URL = apiUrl.endsWith('/api') ? apiUrl.slice(0, -4) : apiUrl;

// Hàm lấy Headers cho Axios (Tự động đính kèm Token)
const getHeaders = (isMultipart = false) => {
    // Ưu tiên admin_token (như bên dashboard), fallback sang adminToken
...
// DANH SÁCH CATEGORY CỐ ĐỊNH (Khớp với client)
const CATEGORIES = [
...
const requireLogin = () => {
    if (!currentUser.value.id) {
        Swal.fire({
            ...
            text: 'Phiên làm việc hết hạn hoặc chưa đăng nhập.',
...
const handleCopyLink = (slug) => {
    const url = `${FRONTEND_URL}/tin-tuc/${slug}`;
...
const handleShare = async (newsItem) => {
    const shareData = { title: newsItem.title, text: newsItem.excerpt, url: `${FRONTEND_URL}/tin-tuc/${newsItem.slug}` };
...
const exportToPDF = (item = null) => {
    ...
    // Nạp html2pdf.js tự động qua CDN để tránh lỗi build từ Vite
    if (typeof window.html2pdf === 'undefined') {
        const script = document.createElement('script');
        script.src = 'https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js';
        script.onload = () => {
            window.html2pdf().set(opt).from(element).save().then(() => Swal.close()).catch(() => Swal.fire('Lỗi', 'Không thể tạo PDF', 'error'));
        };
...
async function fetchNews() {
    ...
    try {
        ...
        // Handle in case backend wraps in { data: [...] }
...
async function handleSave() {
    ...
    // Nếu là Edit, Laravel yêu cầu truyền _method=PUT qua dạng POST
    if (isEditMode.value) payload.append('_method', 'PUT');
    
    const url = isEditMode.value ? `${apiUrl}/admin/news/${formData.id}` : `${apiUrl}/admin/news`;
    
    try {
        await axios.post(url, payload, { headers: getHeaders(true) });
                            ...
                            <form @submit.prevent="handleSave" id="newsForm">
                                <div class="row">
                                    <div class="col-lg-8">
                                        ...
                                        <!-- [CẬP NHẬT] Đã thay thế trình soạn thảo Quill bằng Textarea chuẩn -->
                                        <div class="mb-3">
                                            <label class="form-label required fw-bold mb-2 text-brand">Nội dung chi tiết</label>
                                            <textarea class="form-control" :class="{ 'is-invalid': errors.content }" rows="15" 
                                                v-model="formData.content" 
                                                placeholder="Nhập nội dung bài viết ở đây... (Hỗ trợ nhập mã HTML)"></textarea>
                        ...
                        <div class="container-fluid d-flex justify-content-between align-items-center">
                            <div class="d-flex align-items-center gap-3">
                                <h5 class="modal-title fw-bold text-brand mb-0 text-truncate" style="max-width: 400px;">{{
                                    viewingNewsItem.title }}</h5>
                                <span :class="['badge', getStatusInfo(viewingNewsItem.status).class]">{{
                                        getStatusInfo(viewingNewsItem.status).text }}</span>
                                <span class="badge bg-light text-secondary border"><i class="bi bi-eye me-1"></i>{{ viewingNewsItem.views || 0 }} Views</span>
                            </div>
                        ...
                        <div class="article-container mx-auto bg-white p-4 p-md-5 my-4 shadow-sm rounded">
                            <div class="text-center mb-5">
                                ...
                                <div class="d-flex justify-content-center align-items-center text-muted gap-3">
                                    <span><i class="bi bi-person-circle text-primary"></i> {{ getDisplayAuthor(viewingNewsItem) }}</span>
                                    <span>&bull;</span>
                                    <span><i class="bi bi-calendar3"></i> {{ getFormattedDate(viewingNewsItem.created_at) }}</span>
                                    <span>&bull;</span>
                                    <span><i class="bi bi-eye text-primary"></i> {{ viewingNewsItem.views || 0 }} lượt xem</span>
                                </div>
                            </div>
