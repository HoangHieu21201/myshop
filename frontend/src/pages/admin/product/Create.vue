<template>
  <div class="product-create-wrapper pb-5 mb-5">
            ...
            <div v-show="currentStep === 1" class="row g-4">
                ...
                <div class="p-4 bg-light rounded-4 border h-100">
                  ...
                  <div class="row g-3">
                    <div class="col-md-12">
                      ...
                      <input type="text" class="form-control form-control-lg" v-model="form.name" @input="generateSlug" required placeholder="VD: Nhẫn đính hôn Kim Cương tự nhiên">
                    </div>
                    <div class="col-md-12">
                    ...
                    <div class="col-md-12 mt-3">
                       <div class="alert alert-info small border-0 bg-info bg-opacity-10 text-muted m-0">
                          ...
                          Sản phẩm mặc định sẽ ở trạng thái <strong>Nháp (Draft)</strong>. Bạn cần cấu hình biến thể ở Bước 2 để xuất bản.
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="col-lg-4">
                <div class="p-4 bg-light rounded-4 border text-center h-100">
                  ...
                  <label for="thumbUpload" class="btn btn-outline-brand rounded-pill w-100 fw-semibold"><i class="bi bi-upload me-1"></i> Chọn ảnh chính</label>
                </div>
              </div>
            ...
            <div v-show="currentStep === 2">
                ...
                <div class="card-header bg-white border-bottom p-3 d-flex justify-content-between align-items-center flex-wrap gap-3">
                    ...
                    <div class="attr-toolbar d-flex align-items-center gap-2">
                        <div class="input-group input-group-sm">
                            ...
                            <button type="button" class="btn btn-success px-3 fw-bold" title="Thêm cột vào bảng" @click="addAttributeColumn">
                        ...
                        <button type="button" class="btn btn-sm btn-outline-secondary border-0 fw-bold" @click="openModal('manageAttrModal')">
                        ...
                        <table class="table table-bordered mb-0 variant-table w-100">
                            <thead>
                                <tr>
                                    ...
                                    <!-- RENDER ĐỘNG CÁC CỘT THUỘC TÍNH ĐÃ CHỌN -->
                                    <th v-for="attrId in activeAttributes" :key="attrId" style="min-width: 130px;" class="bg-light text-dark position-relative">
                                      ...
                                      <i class="bi bi-x-circle-fill text-danger position-absolute top-50 end-0 translate-middle-y me-2 cursor-pointer opacity-50 hover-opacity-100" title="Gỡ cột" @click="removeAttributeColumn(attrId)"></i>
                                    </th>
                            ...
                            <tbody>
                                ...
                                <tr v-else v-for="(v, index) in variants" :key="index" class="variant-row" :class="{'row-error': v.hasDuplicateError}">
                                    ...
                                    <!-- SKU -->
                                    <td>
                                      <input type="text" class="form-control form-control-sm font-monospace" v-model="v.sku" placeholder="Tự động" required>
                                    </td>

                                    <!-- CÁC Ô SELECT THUỘC TÍNH -->
                                    ...
                                    <!-- Thông số kinh doanh -->
                                    <td>
                                      <input type="number" class="form-control form-control-sm text-end fw-bold text-brand" :class="{'is-invalid': v.priceError}" v-model="v.price" min="0" required @input="validateRow(index)">
                                    </td>
                                    <td>
                                      <input type="number" class="form-control form-control-sm text-end" :class="{'is-invalid': v.saleError}" v-model="v.promotional_price" min="0" @input="validateRow(index)">
                                    </td>
                                    <td>
                                      <input type="number" class="form-control form-control-sm text-center" v-model="v.stock_quantity" min="0" required>
                                    </td>
                                    <td class="text-center">
                                      <button type="button" class="btn btn-sm text-secondary border-0 hover-danger" @click="removeVariantRow(index)">
                ...
                <div class="card-footer bg-white py-3 d-flex justify-content-between align-items-center sticky-bottom shadow-sm">
                    ...
                    <div class="d-flex align-items-center gap-3">
                        <div class="form-check form-switch m-0">
                          ...
                          <label class="form-check-label fw-semibold" for="publishSwitch">Xuất bản ngay</label>
                        </div>
                    </div>
                </div>
              </div>
              ...
              <div class="d-flex justify-content-between pt-2">
                ...
                <button type="submit" class="btn btn-brand text-white px-5 py-2 fw-bold shadow" :disabled="isSaving || variants.length === 0">
                  <span v-if="isSaving" class="spinner-border spinner-border-sm me-2"></span> HOÀN TẤT LƯU
                </button>
              </div>
    ...
    <!-- 2. Modal Thêm Giá trị "On-the-fly" -->
    ...
    <!-- 3. Modal Quản lý Thuộc tính (Sửa/Xóa) -->
...
// ================= LOGIC MA TRẬN BIẾN THỂ (GRID LOGIC) =================
...
const submitCreateAttribute = async () => {
    ...
    try {
        const res = await fetch('http://127.0.0.1:8000/api/admin/attributes', {
            method: 'POST',
            headers: { ...getHeaders(), 'Content-Type': 'application/json' },
            body: JSON.stringify({ name: newAttrForm.value.name })
        });
        const data = await res.json();
        if (res.ok) {
            data.data.values = []; 
    ...
    try {
        ...
        if (res.ok) {
            ...
            if(currentOperatingRowIndex.value !== null) {
                variants.value[currentOperatingRowIndex.value].attributes[attrObj.id] = data.data.id;
            }
            hideModals();
            validateDuplicates();
...
const updateAttribute = async (id) => {
    ...
    try {
        const res = await fetch(`http://127.0.0.1:8000/api/admin/attributes/${id}`, {
            method: 'PUT',
            headers: { ...getHeaders(), 'Content-Type': 'application/json' },
            body: JSON.stringify({ name: manageAttrName.value })
        });
        if(res.ok) {
...
const submitProduct = async () => {
    ...
    try {
        ...
        const data = await res.json();
        if (res.ok) {
            Swal.fire({ icon: 'success', title: 'Hoàn tất Xuất bản', text: data.message, timer: 2000, showConfirmButton: false }).then(() => {
