                ...
                <div class="card border-0 shadow-sm rounded-4 text-center p-4 mb-4">
                    <div class="position-relative d-inline-block mx-auto mb-3">
                        <img :src="previewAvatar || adminData.avatar"
                            ...
                            style="width: 150px; height: 150px;" alt="Admin Avatar">
                        ...
                        <input type="file" id="avatarUpload" class="d-none" accept="image/png, image/jpeg, image/jpg"
            ...
            <div class="col-md-8 col-lg-9">
                <div class="card border-0 shadow-sm rounded-4 h-100">
                    <div class="card-body p-4 p-lg-5">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input type="tel" class="form-control form-control-lg fs-6"
                                            v-model="adminData.phone" required>
                                    </div>
                                    ...
                                    <!-- Địa chỉ "Xịn Xò" (Chọn API Hành chính) -->
                        ...
                        <!-- TAB 2: ĐỔI MẬT KHẨU CÓ SHOW/HIDE -->
                        <div v-if="activeTab === 'password'">
                            ...
                            <form @submit.prevent="updatePassword">
                                <div class="row">
                                    <div class="col-lg-8">
                                        <div
                                            class="alert alert-warning border-0 bg-warning bg-opacity-10 text-dark mb-4 rounded-3">
                                            <i class="bi bi-shield-lock-fill me-2 text-warning fs-5 align-middle"></i>
                                            Vui lòng tạo mật khẩu mạnh bao gồm chữ cái và số để bảo vệ an toàn cho tài
                                            khoản quản trị của bạn.
                                        </div>

                                        <!-- Input Mật khẩu mới có nút xem -->
                                        <div class="mb-4">
                                            ...
                                            <div class="position-relative">
                                                ...
                                                <button type="button"
                                                    ...
                                                    <i class="bi"
                                                        :class="showNewPassword ? 'bi-eye-slash' : 'bi-eye'"></i>
                                                </button>
                                            </div>
                                        </div>

                                        <!-- Input Xác nhận mật khẩu mới có nút xem -->
                                        <div class="mb-4">
                                        ...
                                        <div class="mt-4">
                                            <button type="submit"
                                                class="btn btn-brand px-5 py-2 fw-bold text-white shadow-sm"
                                                :disabled="isLoading">
                                                <span v-if="isLoading"
...
const handleAvatarChange = (event) => {
    ...
    if (file) {
        if (!['image/jpeg', 'image/png', 'image/jpg'].includes(file.type)) {
            Swal.fire('Lỗi', 'Chỉ chấp nhận file ảnh (JPG, PNG)', 'error'); return;
        }
...
const updatePassword = async () => {
    ...
    try {
        const token = localStorage.getItem('admin_token');
        const response = await fetch('http://127.0.0.1:8000/api/admin/profile/password', {
            method: 'PUT',
        ...
        const data = await response.json();
        if (response.ok) {
            Swal.fire({ icon: 'success', title: 'Thành công', text: 'Mật khẩu đã được đổi. Vui lòng đăng nhập lại.', confirmButtonColor: '#009981' })
                .then(() => {
                    localStorage.removeItem('admin_token'); localStorage.removeItem('admin_info'); localStorage.removeItem('admin_role');
                    window.location.href = '/admin/login';
                });
        } else {
            Swal.fire('Lỗi', data.message || 'Thay đổi thất bại', 'error');
        }
    } catch (error) {
        Swal.fire('Lỗi', 'Không thể kết nối máy chủ', 'error');
    } finally {
